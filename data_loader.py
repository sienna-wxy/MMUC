...
class DataLoader(object):
    def __init__(self, dataset, parameter, step='train'):
        ...
        if step != 'train':
            ...
            for rel in self.all_rels:
                self.eval_triples.extend(self.tasks[rel][self.few:])  # Evaluate query triples in the task
    ...
    def next_one(self):
        ...
        curr_rel = self.all_rels[self.curr_rel_idx]  # Take the current relation
        ...
        # Consistent with GMUC, relation with a small number of candidate entities are removed
        while len(curr_cand) <= 20:
        ...
        curr_tasks_idx = np.random.choice(curr_tasks_idx, self.few + self.nq)  # support + query triples
        support_triples = [curr_tasks[i] for i in curr_tasks_idx[:self.few]]
        query_triples = [curr_tasks[i] for i in curr_tasks_idx[self.few:]]

        # construct support and query negative (1:1) triples
